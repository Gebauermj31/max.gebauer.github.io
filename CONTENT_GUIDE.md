...
project_url: https://canonical-methods-site.example
repository_url: https://github.com/owner/repository
document_url: /files/approved-project-document.pdf
...
Use `document_url` only for an approved, privacy-reviewed public document. Omit repository fields entirely until a public repository exists; do not publish placeholder or “coming soon” links.

For FAIR xwOBA, the release label, evidence status, summary, figure, canonical URL, and repository URL form one synchronized release unit. Never update only one of them. Sprint Speed, player identity, and player traits remain outside the predictor and may enter only after predictions are written and hashed. Pre-pitch defensive alignment is an allowed context variable whose indirect hitter information must be disclosed.
...
- Add personal photographs through `_data/photos.yml`; provide an 800-pixel and 1600-pixel JPEG/WebP pair, descriptive alt text, a concise caption, and a credit line.
- Treat personal-photo clearance as permission for this website, not as a general reuse license. Obtain and document consent before adding an identifiable person who has not already approved publication.
...
3. Confirm that each personal photograph is approved and every identifiable person depicted has consented to publication.
4. Build with strict front matter and run `scripts/check_site.rb`.
5. Review mobile, tablet, and desktop layouts in light and dark themes.
6. Review keyboard order, focus states, headings, alt text, captions, and contrast.
